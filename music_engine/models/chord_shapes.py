...
This module provides a comprehensive database of real guitar chord shapes
based on the CAGED system. Includes open chords, barre chords, triads, and
extended chords with proper fingering and voicings.
"""
...
class ChordShape:
    ...
    name: str
    quality: str
    frets: List[Optional[int]]  # None=muted, 0=open, n=fret n
...
# Note: frets stored from low E (string 6) to high E (string 1)
...
def get_chord_shape(root: str, quality: str) -> List[ChordShape]:
    ...
    for barre_key, barre_shape in BARRE_CHORDS.items():
        if barre_shape.quality == quality_norm:
            # Transpose barre chord based on root
...
def transpose_shape(shape: ChordShape, target_fret: int) -> ChordShape:
    ...
    for f in shape.frets:
        if f is None:
            ...
            new_frets.append(f + base_diff)
    
    return ChordShape(
        name=shape.name,
        quality=shape.quality,
        frets=new_frets,
        fingers=shape.fingers.copy(),
        base_fret=target_fret,
        is_barre=True,
        root_string=shape.root_string,
        position_name=f"{shape.position_name} (fret {target_fret})"
    )
